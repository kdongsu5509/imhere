...
- 🗺️ **지오펜스 관리**: 특정 위치와 반경을 설정하여 지오펜스 영역을 등록하고 관리
...
현재 프로젝트는 수동 배포 방식을 사용하고 있습니다. 향후 GitHub Actions 또는 Jenkins를 통한 자동화된 CI/CD 파이프라인 구성을 권장합니다.

### c. 모니터링 및 로깅 구성

- **애플리케이션 로그**: Spring Boot 기본 로깅 사용
- **에러 추적**: 예외 처리 핸들러를 통한 에러 로깅
- **성능 모니터링**: 향후 APM 도구 도입 권장 (예: New Relic, Datadog)
...
- Hexagonal Architecture (포트-어댑터 패턴) 사용
...
이 프로젝트는 우아한테크코스 8기 프리코스 오픈미션의 과제로 수행된 프로젝트입니다.
